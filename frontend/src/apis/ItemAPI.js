...
const baseUrl =
  process.env.REACT_APP_BACKEND_PRODUCT_SERVICE ;
...
const itemsAPI = {
  items() {
    return {
      ...
      create: (newItem) =>
        axios.post(baseUrl + "/api/products/", newItem, config),
      ...
      fetchItemsById: (id) =>
        axios.get(baseUrl + "/api/products/" + id, config),
      updateStockSeller: (qty, id) =>
        axios.put(baseUrl + "/api/item/sellar/" + qty + "/" + id, config),
      fetchitemstockbybrandforuser: (id) =>
        axios.get(baseUrl + "/api/item/itembrands/" + id, config),
    };
  },
};
